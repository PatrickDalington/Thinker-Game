...
public class EmojisDataProvider extends AbstractDataProvider {
    ...
    public EmojisDataProvider() {
        ...
        String[] emojis = {"😻", "🧒🏻", "🐇", "👨🏻‍💼"};
        List<String> emj;
        emj = Arrays.asList(emojis);
        Collections.shuffle(emj);
        ...
        for (int i = 0; i < 1; i++) {
            for (int j = 0; j < t.length(); j++) {
                ...
                final int emojiRandom = new Random().nextInt((emojiMax - min) + 1) + min;
                ...
                final String emoji = emj.get(emojiRandom);
                final int swipeReaction = RecyclerViewSwipeManager.REACTION_CAN_SWIPE_UP | RecyclerViewSwipeManager.REACTION_CAN_SWIPE_DOWN;
                mData.add(new ConcreteData(charPos, id, viewType, text, emoji, swipeReaction, color));
            }
        }
    ...
    public static final class ConcreteData extends Data {
        ...
        ConcreteData(int charPosition, long id, int viewType, @NonNull String text, String emoji, int swipeReaction, int color) {
            mId = id;
